                    ...
                    <form action="{{ route('institutions.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div>
                                ...
                                <select name="type" id="type" 
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                                    <option value="" @if(old('type') == '') selected @endif>Seleccionar Tipo</option>
                                    <option value="school" @if(old('type') == 'school') selected @endif>Escuela</option>
                                    <option value="college" @if(old('type') == 'college') selected @endif>Colegio/Universidad</option>
                                    <option value="club" @if(old('type') == 'club') selected @endif>Club Deportivo</option>
                                    <option value="federation" @if(old('type') == 'federation') selected @endif>Federación Deportiva</option>
                                    <option value="academy" @if(old('type') == 'academy') selected @endif>Academia Deportiva</option>
                                    <option value="other" @if(old('type') == 'other') selected @endif>Otro</option>
                                </select>
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="3" 
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">{{ old('description') }}</textarea>
